  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 bg-background min-h-screen text-primary">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-12 bg-surface border border-border rounded-[24px] p-6 md:p-8 shadow-sm">
        ...
        <div className="bg-surface-raised rounded-2xl p-8 flex items-center justify-center border border-subtle min-h-[350px] max-h-[450px] overflow-hidden">
          <img 
            src={product.image_url || `https://ui-avatars.com/api/?name=${encodeURIComponent(product.name)}&size=600&background=a855f7&color=fff&bold=true&font-size=0.2`} 
            alt={product.name} 
          ...
          <div className="mt-8 border-t border-subtle pt-6">
            <div className="flex gap-4">
              {user ? (
                ...
                <Link to="/login" className="btn-primary flex-1 py-3 justify-center text-sm font-semibold rounded-xl text-center">
                  Log in to Purchase
                </Link>
              )}
