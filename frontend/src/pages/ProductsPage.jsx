...
// ─── ACOUSTIC & HAPTIC FEEDBACK UTILITY ──────────────────────────────
const triggerFeedback = () => {
  // 1. Acoustic Feedback (Web Audio API Synth Click)
  try {
    ...
    if (AudioContext) {
      ...
      osc.type = 'sine'
      osc.frequency.setValueAtTime(400, ctx.currentTime) // Clean click tone
  ...
  // 2. Haptic Feedback (Vibration API for mobile touch displays)
  if ('vibrate' in navigator) {
    navigator.vibrate(8) // Quick micro haptic pulse
  }
}

// Helper to match a fallback categories icon indicator
...
export default function ProductsPage() {
  ...
  // Grid layout switcher options: 'density-dense' (2 cols), 'density-modern' (4 cols), 'density-list' (compact row view)
  ...
  useEffect(() => {
    Promise.all([productsApi.list(), categoriesApi.list()]).then(([p, c]) => {
      ...
      const max = Math.max(...p.data.map(pr => pr.price), 1000)
      setMaxPrice(max)
      setPriceRange([0, max])
      setLoading(false)
  ...
  // Live real-time counter metrics calculated independently per specific option state
  const liveCountMetrics = useMemo(() => {
    const counts = { all: 0 }
    categories.forEach(c => { counts[c.id] = 0 })

    products.forEach(p => {
      // Validate search criteria matches
      const matchesSearch = !search || 
        p.name.toLowerCase().includes(search.toLowerCase()) || 
        p.description.toLowerCase().includes(search.toLowerCase())
      
      // Validate price criteria matches
      const matchesPrice = p.price >= priceRange[0] && p.price <= priceRange[1]

      if (matchesSearch && matchesPrice) {
        counts.all++
        if (p.category_id in counts) {
  ...
  // Determine container styles dynamically relative to layout variables
  const gridContainerClass = useMemo(() => {
    if (gridVariant === 'density-dense') return "grid grid-cols-1 sm:grid-cols-2 gap-8"
    if (gridVariant === 'density-list') return "flex flex-col gap-3"
    return "grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5" // density-modern
  }, [gridVariant])
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-1 p-1 rounded-xl bg-surface border" style={{ background: 'var(--surface)', borderColor: 'var(--border)' }}>
            {[
              { id: 'density-dense', label: 'Large Grid', icon: Grid2X2 },
              { id: 'density-modern', label: 'Modern Grid', icon: Grid3x3 },
              { id: 'density-list', label: 'List View', icon: LayoutList },
            ].map(item => {
              ...
              return (
                <button
                  key={item.id}
                  onClick={() => handleVariantSwitch(item.id)}
          ...
          <button onClick={() => { triggerFeedback(); setSidebarOpen(true); }} className="btn-secondary text-xs py-2.5 lg:hidden">
            ...
            {activeFilters.length > 0 && (
              <span className="ml-1 w-4 h-4 rounded-full text-white text-[10px] flex items-center justify-center"
                style={{ background: '#a855f7' }}>{activeFilters.length}</span>
            )}
          </button>
        </div>
      </div>
      ...
      <div className="flex gap-8">
        ...
        <aside className="hidden lg:block w-64 flex-shrink-0 sticky top-24 self-start">
          <div className="p-5 rounded-2xl animate-fade-in" style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}>
            ...
            <div className="space-y-6">
              ...
              {/* Acoustic & Haptic Visual Token Chips Container */}
              <div>
              ...
              <button
                onClick={() => { triggerFeedback(); setSelectedCategory(null); setPriceRange([0, maxPrice]); setSearch(''); setSort('default') }}
        ...
        {/* Mobile Sidebar Overlay Screen Drawer */}
        <AnimatePresence>
          {sidebarOpen && (
            <>
              ...
              >
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <div className="flex flex-col gap-1.5">
                      <button
                        onClick={() => { handleCategoryClick(null); setSidebarOpen(false); }}
                      ...
                      {categories.map(cat => {
                        ...
                        return (
                          <button
                            key={cat.id}
                            onClick={() => { handleCategoryClick(isSelected ? null : cat.id); setSidebarOpen(false); }}
                  ...
                  <button
                    onClick={() => { triggerFeedback(); setSelectedCategory(null); setPriceRange([0, maxPrice]); setSearch(''); setSort('default'); setSidebarOpen(false); }}
                    className="w-full btn-ghost text-xs text-red-400 hover:text-red-500 justify-center"
                  >
        ...
        {/* Dynamic Variant Products Viewport Grid */}
        <div className="flex-1 min-w-0">
          {loading ? (
            ...
            <EmptyState icon={Package} title="No products found" description="Try adjusting your active target filters matrix."
              action={
                <button onClick={() => { triggerFeedback(); setSelectedCategory(null); setPriceRange([0, maxPrice]); setSearch('') }} className="btn-primary">
                  Reset Grid Matrix
                </button>
              }
            />
            ...
            >
              <AnimatePresence mode="popLayout">
                {filtered.map((product, i) => (
                  ...
                  >
                    {/* Render standard visual or a structural alternative for list layouts */}
                    {gridVariant === 'density-list' ? (
                      ...
                      >
                        ...
                        <div className="text-right flex-shrink-0 ml-4">
                          <span className="text-sm font-bold text-gradient">₹{product.price.toLocaleString()}</span>
